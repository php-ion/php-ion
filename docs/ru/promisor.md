...
Базовый промис который хранит набор коллбеков когда промис будет выполнен. 
Однако в отличии `ION\ResolvablePromise` базоый промис `ION\Promise` не может выполнить себя.
Этот проаис имеет очень узкое применение и используется в цепочках действий.
...
Отложенное (deferred) дейстиве или результат. Большинство методов расширения возвращают deferred.
Конструтор deferred, опционально, принимает коллбек отмены. 
Коллбек отмены нужне для выполнения какого-то кода отката или отмены состояний при отмене deferred. 
